...
import { GetAnotherUserInfo } from "../api/user";
...
const MatchDetail = () => {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => GetAnotherUserInfo(Number(id)),
  });
  if (data)
    return (
      <div>
        {isModalOpen && <ImgModal src={data.imgUrls} onModal={onModal} />}
        {isMessageOpen && (
          <InputModal
            value={inputVal}
        ...
        >
          ...
          <div className="z-10 mt-auto">
            <span className="text-white text-displayAccent">
              {data.username}
            </span>
            <span className="mb-1 ml-1 text-white text-titleDefault">
              {calculateAge(data.birth)}살
            </span>
          </div>
          <MessageBtn onClick={parentClick} />
        </BgImg>
        <div className="flex flex-col gap-2 p-4">
          <p className="text-titleAccent">자기소개</p>
          <p className="text-captionDefault">
            안녕하세요! 매일을 특별하게 만드는 순간을 사랑하는 사람입니다.
            귀여운 물건을 수집하거나 예쁜 카페를 탐방하는 게 취미예요. 여행
            계획을 세우는 것도 좋아하고, 상상력과 따뜻함이 제 장점입니다. 소소한
            일상부터 인생 이야기까지 함께 나누며 즐거운 시간을 보낼 수 있는 분을
            만나고 싶어요. 우리 함께 작은 행복들을 찾아볼까요? 😊
          </p>
          <div className="flex gap-1 overflow-x-auto">
            {data.imgUrls.map((item, idx) => (
              <button
                ...
                key={idx}
              >
                <img src={item.imgUrl} className="shrink" />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 p-4">
          <div className="flex items-center gap-1 ">
            <div className="w-[80px] flex gap-1 items-center">
              ...
              <span className="text-captionDefault text-contents-default-tertiary">
                학교
              </span>
            </div>
            <span className="truncate text-captionDefault text-contents-default-primary">
              계원예술대학교 디지털미디어디자인과
            </span>
          </div>
          <div className="flex items-center gap-1 ">
